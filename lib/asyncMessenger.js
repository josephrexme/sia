...
const AsyncMessenger = function(messenger, macros, onReady) {
  ...
  self.sendMessage = function(user, message, channel) {
    const reply = self.rs.reply(user, message)
    if(reply.then){
      reply.then(function(replyStr){
  ...
  self.rs.loadDirectory('./brain', function() {
    self.rs.sortReplies()
    onReady()
  })

  Object.keys(macros).forEach((macro) => {
    console.log(`adding macro ${macro}`)
    self.rs._objlangs[macro] = 'javascript'
    self.rs._handlers.javascript._objects[macro] = function(rs, args){
